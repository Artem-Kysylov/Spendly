    ...
    (async () => {
      ...
      if (event.data) {
        try {
          ...
          try {
            payload = { message: await event.data.text() };
          } catch {}
        }
      }
      ...
      const renotify =
        typeof payload.renotify === "boolean" ? payload.renotify : true;
      const data = {
        deepLink:
          (payload.data && payload.data.deepLink) ||
          payload.deepLink ||
          "/dashboard",
      };
      ...
      if (Notification.permission !== "granted") {
        ...
        for (const client of clientsList)
          client.postMessage({ type: "DEVTOOLS_PUSH", message: body });
        return;
      }

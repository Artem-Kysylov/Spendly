...
test("Free: Add button disabled after creating two recurring rules", async ({
  page,
}) => {
  await page.goto("/en");
  ...
  await expect(
    page.getByRole("heading", { name: "Recurring Rules" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      "In Free plan, you can add up to 2 recurring rules. Upgrade to Pro for unlimited.",
    ),
  ).toBeVisible();
});

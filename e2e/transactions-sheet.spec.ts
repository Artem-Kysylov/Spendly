...
test('Mobile: открытие/закрытие шторки транзакции', async ({ page }) => {
  ...
  await page.evaluate(() => window.dispatchEvent(new CustomEvent('transactions:add')))
...
test('Mobile: вложенная шторка календаря', async ({ page }) => {
  ...
  // Открыть календарь
  await page.getByRole('button', { name: /Pick a date|Choose date|Pick up the date/i }).click()

  // Заголовок вложенной шторки
  await expect(page.getByRole('heading', { name: /Choose date/i })).toBeVisible()

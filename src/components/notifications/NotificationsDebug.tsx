...
export default function NotificationsDebug() {
  ...
  const devtoolsPushHint = () => {
    append('Если DevTools Push ничего не делает, убедитесь, что Notification.permission = granted и push-обработчик SW действительно срабатывает. Предупреждение Workbox InjectManifest в dev — ожидаемое.')
  }
  ...
  return (
    <div style={{ padding: 12, border: '1px solid #ddd', borderRadius: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
        <button onClick={requestPermission} style={{ padding: '6px 10px' }}>Request permission</button>
        <button onClick={subscribePush} style={{ padding: '6px 10px' }}>Subscribe to push</button>
        <button onClick={sendQueueItem} style={{ padding: '6px 10px' }}>Send test queue item</button>
        <button onClick={devtoolsPushHint} style={{ padding: '6px 10px' }}>Show hints</button>
      </div>
      <pre style={{ marginTop: 12, maxHeight: 240, overflow: 'auto', background: '#f7f7f7', padding: 10 }}>{log || 'No logs yet'}</pre>
    </div>
  )
}
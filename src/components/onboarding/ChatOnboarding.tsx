...
export default function ChatOnboarding() {
  ...
  // Budget style state
  const [selectedBudgetStyle, setSelectedBudgetStyle] =
    useState<BudgetStyle | null>(null);
  ...
  useEffect(() => {
    ...
    if (manuallySelected) {
      ...
      const suggestedCurrency =
        LANGUAGE_CURRENCY_MAP[manuallySelected as Language] || "USD";
  ...
  }, [currentLocale, router]); // Empty deps - run only once on mount
          ...
          setMessages([
            {
              id: "greeting",
              role: "assistant",
              content: t("step0.currency_greeting"),
            },
          ]);
        },
        800 + Math.random() * 400,
  ...
  const handleCurrencySelect = useCallback(
    (currencyCode: string, flag: string) => {
  ...
  const handleBudgetStyleSelect = useCallback(
    async (style: BudgetStyle, label: string) => {
      ...
      try {
        const localeRes = await saveUserLocaleSettings({
        ...
        if (!localeRes.success) {
          console.error("[Onboarding] Failed to save locale settings", {
            userId: session.user.id,
            stage: localeRes.stage,
            code: localeRes.code,
            message: localeRes.message,
          });
        ...
        const budgetAmount =
          BUDGET_AMOUNTS[currency]?.[style] || BUDGET_AMOUNTS.USD[style];
        ...
        if (selectedCategory && transactionAmount) {
          if (!isValidAmountInput(transactionAmount)) {
        ...
        setTimeout(async () => {
          ...
          await addBotMessage(
            renderNotificationsMessage(t("notifications_step")),
          );
          setStep(5);
        }, 300);
      } catch (e) {
  ...
  const categories = [
    {
      id: "coffee",
      label: t("step1.action_coffee"),
      icon: Coffee,
      emoji: "☕",
    },
    {
      id: "food",
      label: t("step1.action_food"),
    ...
    {
      id: "groceries",
      label: t("step1.action_groceries"),
      icon: ShoppingCart,
      emoji: "🛒",
    },
  ];
  ...
  const budgetStyles = [
    {
      id: "monk" as BudgetStyle,
      label: t("step3.style_monk"),
      desc: t("step3.style_monk_desc"),
      icon: Wallet,
      emoji: "🧘",
      amount: BUDGET_AMOUNTS[currency]?.monk || BUDGET_AMOUNTS.USD.monk,
    },
    {
      id: "balanced" as BudgetStyle,
      label: t("step3.style_balanced"),
      desc: t("step3.style_balanced_desc"),
      icon: Scale,
      emoji: "⚖️",
      amount: BUDGET_AMOUNTS[currency]?.balanced || BUDGET_AMOUNTS.USD.balanced,
    },
    {
      id: "rich" as BudgetStyle,
      label: t("step3.style_rich"),
      desc: t("step3.style_rich_desc"),
      icon: Sparkles,
      emoji: "💎",
      amount: BUDGET_AMOUNTS[currency]?.rich || BUDGET_AMOUNTS.USD.rich,
    },
  ];

...
export const PresetChipsRow = ({
  ...
  const modeClass =
    scrollbarMode === "never"
      ? "chips-scroll chips-scroll--never"
      : scrollbarMode === "always"
        ? "chips-scroll chips-scroll--always"
        : "chips-scroll chips-scroll--hover";
  return (
    <div className={className ?? ""}>
      <div
        className={`${modeClass} flex flex-wrap gap-2 pb-2 px-4 border-t border-border bg-background`}
      >
        {chips.map((label, idx) => (
          <button
            key={idx}

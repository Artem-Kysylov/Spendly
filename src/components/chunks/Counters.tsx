...
function TransactionsCounters({
  ...
  const incomeTrend = calculatePercentageChange(
    previousMonthIncome,
    totalIncome,
  );
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-[140px] bg-gray-200 animate-pulse rounded-lg"
          ></div>
        ))}
      </div>
    );
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-3 md:p-6">
            ...
            >
              ...
              >
                <div
                  className="absolute top-3 right-3 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10"
                  onClick={onIconClick}
                >
                    ...
                    <>
                      ...
                      <div className="text-center space-y-1">
                        <p className="text-xs text-gray-700 dark:text-white font-medium">
                          ...
                          {tDashboard("counters.remaining")}
                        </p>
                        <p className="text-xs text-gray-600 dark:text-white">
                          {budgetUsagePercentage.toFixed(1)}%{" "}
                          {tDashboard("counters.usedPercent")}
                        </p>
                      </div>
                    </>
                  )}
              ...
              >
                ...
                <div className="text-center space-y-1">
                  ...
                  <p className="text-xs text-gray-600 dark:text-white">
                    {tDashboard("counters.coversExpenses", {
                      percent: incomeCoverage.toFixed(0),
                    })}
                  </p>
                </div>
              </motion.div>
              ...
              <div className="flex-1">
                ...
                {!tipLoading && !displayTip && !tipError && (
                  <span className="text-black dark:text-white text-sm">
                    💡 {tCharts("ai.placeholder")}
                  </span>
                )}
                {tipError && (
                  <p className="text-red-600 text-xs mt-1">{tipError}</p>
                )}
                {isRateLimited && (
                  <p className="text-yellow-600 text-xs mt-1">
                    {tCharts("ai.rateLimited")}
                  </p>
                )}
              </div>
              <button

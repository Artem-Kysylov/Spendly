...
export async function GET(_req: NextRequest) {
  ...
  let ok = false
  let model = provider === 'openai' ? (process.env.OPENAI_MODEL ?? 'gpt-4-turbo') : (process.env.GEMINI_MODEL ?? 'gemini-2.5-flash')
  ...
  try {
    const stream = provider === 'openai'
      ? streamOpenAIText({ model, prompt: 'Health check: respond with "pong".', system: 'You are health-check.', requestId })
      : streamGeminiText({ model, prompt: 'Health check: respond with "pong".', system: 'You are health-check.', requestId })
  ...
  const payload = { ok, provider, model, durationMs, requestId, error, blockReason }

...
export async function POST(req: NextRequest) {
  try {
    ...
    const isPro = ((user.user_metadata || {}) as any)?.subscription_status === 'pro'
    if (!isPro) {
    ...
    const payload = {
      ...
      next_due_date: String(body.next_due_date || new Date().toISOString().slice(0, 10)),
      active: true,
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'updateFailed' }, { status: 400 })

    const patch: any = {}
    if (typeof body.title_pattern === 'string') patch.title_pattern = body.title_pattern
    if (body.cadence === 'weekly' || body.cadence === 'monthly') patch.cadence = body.cadence
    if (typeof body.avg_amount === 'number') patch.avg_amount = body.avg_amount
    if (typeof body.budget_folder_id === 'string' || body.budget_folder_id === null) patch.budget_folder_id = body.budget_folder_id
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const body = await req.json()
    const id = String(body.id || '')
    if (!id) return NextResponse.json({ error: 'deleteFailed' }, { status: 400 })

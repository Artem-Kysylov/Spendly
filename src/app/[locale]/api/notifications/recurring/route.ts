...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: rules, error } = await supabase
      ...
      .select(
        "id, title_pattern, avg_amount, cadence, next_due_date, budget_folder_id, active",
      )
      .eq("user_id", user.id)
      .eq("active", true);
    ...
    for (const r of rules || []) {
      ...
      if (diff <= 0) {
        const title = tNotifications("recurring.dueTodayTitle", {
          name: r.title_pattern,
        });
        ...
        const title = tNotifications("recurring.dueSoonTitle", {
          name: r.title_pattern,
        });
        const message = tNotifications("recurring.message", {
          date: r.next_due_date,
    ...
    return NextResponse.json({
      inserted: { dueToday: dueTodayCount, dueSoon: dueSoonCount },
      date: todayISO,
    });
  } catch (error) {
    const localeCookie =

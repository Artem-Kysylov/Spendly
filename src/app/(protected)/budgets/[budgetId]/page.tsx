...
export async function generateMetadata({ params }: { params: { budgetId: string } }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pages.budgetDetails.meta' })
  const title = budgetName ? t('titleWithName', { name: budgetName }) : t('title')
  const description = budgetName ? t('descriptionWithName', { name: budgetName }) : t('description')

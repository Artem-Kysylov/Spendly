...
export const EN_EMPTY_GENERIC =
  "No expenses recorded for the requested period.";
...
export function localizeEmptyWeekly(
  period: "thisWeek" | "lastWeek",
  locale?: string,
): string {
  const isRu = (locale || "").toLowerCase().startsWith("ru");
  if (period === "thisWeek")
    return isRu ? RU_EMPTY_THIS_WEEK : EN_EMPTY_THIS_WEEK;
  return isRu ? RU_EMPTY_LAST_WEEK : EN_EMPTY_LAST_WEEK;
}

export function localizeEmptyMonthly(
  period: "thisMonth" | "lastMonth",
  locale?: string,
): string {
  const isRu = (locale || "").toLowerCase().startsWith("ru");
  if (period === "thisMonth")
    return isRu ? RU_EMPTY_THIS_MONTH : EN_EMPTY_THIS_MONTH;

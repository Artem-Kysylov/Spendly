...
import { startOfDay, endOfDay, subDays, startOfMonth, endOfMonth, format, isToday, isYesterday } from 'date-fns'
...
export const useTransactionsInfinite = ({ initialType = 'all' }: UseTransactionsInfiniteProps = {}) => {
    ...
    const [filterType, setFilterType] = useState<TransactionFilterType>(initialType)
    ...
    const fetchTransactions = useCallback(async (pageIndex: number, isNew: boolean) => {
        ...
        `)
            ...
            setTransactions(prev => isNew ? transformedData : [...prev, ...transformedData])
        } catch (err) {

    ...
    async (limit = 50, offset = 0, unreadOnly = false) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            (errorData as any).error || "Failed to fetch notifications",
          );
        }
    ...
    async (id: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            (errorData as any).error || "Failed to mark notification as read",
          );
        }
    ...
    async (notificationData: {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            (errorData as any).error || "Failed to create notification",
          );
        }

...
export function useAISuggestions() {
  ...
  const fetchSuggestion = useCallback(
    async (prompt: string) => {
      ...
      const tone = isPro
        ? (session.user.user_metadata as any)?.assistant_tone || "neutral"
        : "neutral";
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (ct.includes("text/html") || res.status === 404) {
            setError(
              "Assistant endpoint is not reachable for this locale. Please reload and try again.",
            );
        ...
        if (
          !isPro &&
          Number.isFinite(dailyLimitHeader) &&
          Number.isFinite(usedHeader)
        ) {
          if (usedHeader === 3) {
            toast({
              title: tAssistant("toasts.usedNOfDailyLimit", {
                used: usedHeader,
                limit: dailyLimitHeader,
              }),
              duration: 5000,

...
export function useAvatarProcessing(targetSize = 256) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const offsetX = Math.floor(((img.naturalWidth || img.width) - minDim) / 2)
      const offsetY = Math.floor(((img.naturalHeight || img.height) - minDim) / 2)
      const canvas = document.createElement('canvas')
      canvas.width = targetSize
      canvas.height = targetSize
      const ctx = canvas.getContext('2d')
      if (!ctx) throw new Error('Failed to create canvas context')
      ctx.drawImage(img, offsetX, offsetY, minDim, minDim, 0, 0, targetSize, targetSize)
      setProgress(60)
      let mime = 'image/webp'
      let blob: Blob
      try {
      ...
      setProgress(100)
      return { blob, previewUrl: url, mime, width: targetSize, height: targetSize }
    } finally {
  ...
  return { isProcessing, error, previewUrl, processedBlob, progress, processFile, clear, abort }
}
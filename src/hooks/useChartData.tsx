...
export const useLineChartData = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const lineData: LineChartData[] = Object.entries(dailyTotals).map(
        ([date, totals]) => {
          const amount =
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch line chart data",
      );
...
export const useBarChartData = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch bar chart data",
      );
      // Fallback to mock data in case of error
...
export const useComparisonLineChartData = (
  filters: ChartFilters,
): UseComparisonChartDataReturn => {
  const { session } = UserAuth();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const currentDailyTotals = groupTransactionsByDay(
        currentResult.data || [],
      );
      const previousDailyTotals = groupTransactionsByDay(
        previousResult.data || [],
      );
      ...
      const comparisonData: ComparisonLineChartData[] = dateRange.map(
        (date) => {
          const dateStr = date.toISOString().split("T")[0];
          const currentAmount = getAmountByDataType(
            currentDailyTotals[dateStr],
            filters.dataType.toLowerCase() as "expenses" | "income",
          );
          const previousAmount = getAmountByDataType(
            previousDailyTotals[dateStr],
            filters.dataType.toLowerCase() as "expenses" | "income",
          );
      ...
      const currentTotal = Object.values(currentDailyTotals).reduce(
        (sum, day) =>
          sum +
          getAmountByDataType(
            day,
      ...
      const previousTotal = Object.values(previousDailyTotals).reduce(
        (sum, day) =>
          sum +
          getAmountByDataType(
            day,
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to fetch comparison chart data",
      );
      setData([]);
    } finally {
      setIsLoading(false);

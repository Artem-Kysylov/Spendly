...
export function budgetTotals(txs: Transaction[], budgetNameById: Map<string, string>): Map<string, number> {
  const totals = new Map<string, number>()
  for (const tx of (txs || [])) {
    if (tx.type !== 'expense') continue
    const budgetName = tx.budget_folder_id ? (budgetNameById.get(tx.budget_folder_id) || 'Unknown') : 'Unassigned'
    totals.set(budgetName, (totals.get(budgetName) || 0) + (Number(tx.amount) || 0))
  }
  return totals
}
...
export function getLastMonthRange(now: Date = new Date()): { start: Date; end: Date } {
...
export function compareMonthTotals(thisMonthTxs: Transaction[], lastMonthTxs: Transaction[]) {

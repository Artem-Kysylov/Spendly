...
export function buildBarChartPrompt(params: {
  ...
  const windowSize = params.windowSize ?? (filters.period === "Week" ? 6 : 6); // можно увеличить на 8 при желании
  ...
  const pct =
    prev === 0
      ? current === 0
        ? 0
        : 100
      : Number(((delta / prev) * 100).toFixed(1));
  const avg = amounts.length
    ? Number((amounts.reduce((a, b) => a + b, 0) / amounts.length).toFixed(1))
    : 0;

...
function tx(title: string, dateISO: string, amount = 9.99, budget: string | null = null) {
  return { title, amount, type: 'expense' as const, budget_folder_id: budget, created_at: dateISO }
}
...
describe('recurring heuristics', () => {
  ...
  it('detects monthly cadence with ±5 day drift', () => {
    const ds = ['2025-01-03', '2025-02-03', '2025-03-06', '2025-04-28'].map(s => new Date(s))
  ...
  it('computes median amount', () => {
    const ds = ['2025-01-01', '2025-01-08', '2025-01-15'].map(s => new Date(s))
  ...
  it('filters false positives (insufficient occurrences)', () => {
    const txs = ['2025-01-01', '2025-01-08'].map(s => tx('RareService', s, 5))
    const c = findRecurringCandidates(txs)
